...
failedOrderCheckRouter.get("/:orderIdOrRef", async (req, res) => {
  ...
  const orderIdOrRef = String(req.params.orderIdOrRef || "").trim();
  ...
  if(!orderIdOrRef){
    ...
    try{
      const base = (
        process.env.SWIFT_BASE_URL || "https://swiftdata-link.com"
      ).replace(/\/$/, "");
      ...
      axios.get(swiftUrl, {
        headers: {
          "x-api-key":
          process.env.SWIFT_API-KeyboardEvent,
      ...
      console.error("Live Order Status Search Error:",
      ...
      if(error.response){
        return res.status(500 || error.response.status).json({
          success: false,
          message: error.response.data?.message || "Unable to retrieve order status."
        });
      }
