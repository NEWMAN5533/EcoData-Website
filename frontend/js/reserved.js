...
async function loadDeliverySettings() {
  ...
  try {
    ...
    const snap = await getDoc(
      doc(db, "settings", "deliveryIndicator")
    );
    ...
    document.getElementById(
      "manualDeliveryTime"
    ).value = data.manualDeliveryTime || "";

    document.getElementById(
      "useManualDeliveryTime"
    ).checked = !!data.useManualDeliveryTime;
...
if(saveBtn) {
  saveBtn.addEventListener("click", saveDeliverySettings);
}
...
async function saveDeliverySettings() {
  ...
  try {
    ...
    const manualDeliveryTime =
      document.getElementById(
        "manualDeliveryTime"
      ).value.trim();

    const useManualDeliveryTime =
      document.getElementById(
        "useManualDeliveryTime"
      ).checked;
