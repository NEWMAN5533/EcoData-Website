...
// ====AGENT CODE=================
async function loadAgentPricing() {
  try {
    if (!storeId) {
      console.log("No store → default pricing");
      applyDefaultPrices();
      return;
    }
    ...
    if (!snap.exists()) {
      console.log("Store not found → fallback");
      applyDefaultPrices();
      return;
    }

    const pricing = snap.data().pricing || {};

    applyAgentPrices(pricing);
    updateOptionSelect(pricing);
...
function applyAgentPrices(pricing) {
  document.querySelectorAll(".bundle-card").forEach(card => {
    const size = card.dataset.size;
    const network = card.dataset.network.toLowerCase();

    const key = `${network}_${size}`;
    const agentPrice = pricing[key];

    const base = Number(card.dataset.baseprice || 0);
    const finalPrice = agentPrice || base + DEFAULT_MARKUP;

    updateCard(card, finalPrice);
  });
}
...
function applyDefaultPrices() {
  document.querySelectorAll(".bundle-card").forEach(card => {
    const base = Number(card.dataset.baseprice || 0);
    updateCard(card, base + DEFAULT_MARKUP);
  });
}
...
function updateOptionSelect(pricing = {}) {

  document.querySelectorAll(".optionSelect").forEach(option => {

    const size = option.dataset.size;
    const network = option.dataset.network.toLowerCase();

    const key = `${network}_${size}`;
    const agentPrice = pricing[key];

    const base = Number(option.dataset.baseprice || 0);
    const finalPrice = agentPrice || base + DEFAULT_MARKUP;
    ...
    // ✅ Update UI
    const priceEl = option.querySelector("small");
    if (priceEl) {
      priceEl.textContent = `GHS ${finalPrice}`;
    }
  });
}
...
document.querySelectorAll(".optionSelect").forEach(opt => {
  opt.addEventListener("click", () => {
    ...
    if (networkHolder) {
      networkHolder.textContent = selectedBundle.network.toUpperCase();
    }
...
selectBtn.addEventListener("click", (e) => {
  e.stopPropagation();
  dropdownOffer.classList.add("show");
});
