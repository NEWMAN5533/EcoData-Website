...
import { getStorePricing, calculatePrice } from "./store.js";
...
function resolvePackageName(network) {
  ...
  // ApplyPriceToUI
  function applyPriceToUI(pricing) {
    ...
    document.querySelectorAll(".gridCard").forEach(card => {
      ...
      const finalPrice = calculatePrice(pricing, network, size, base);

      // Update the UI
      const priceEl = card.querySelector(".old-price");
      if(priceEl) priceEl.textContent = `GHS ${finalPrice}`;

      // Update button dataset
      const btn =  card.querySelector(".buy-btn");
      if(btn) btn.dataset.price = finalPrice;
    });
...
document.addEventListener("DOMContentLoaded", async function() {
  STORE_PRICING = await getStorePricing();

  applyPriceToUI(STORE_PRICING);
})
